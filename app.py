...
def apply_fig_style(fig, height=380, legend=True):
    """Applique le design system uniformément à tous les graphiques Plotly."""
    fig.update_layout(
        height=height,
        ...
        hovermode="x unified",
        hoverlabel=dict(bgcolor=PC["text"], font_color="white", font_size=11,
                        bordercolor=PC["text"]),
        legend=dict(
            orientation="h", yanchor="bottom", y=1.01, xanchor="right", x=1,
            font_size=10, bgcolor="rgba(255,255,255,0.9)",
        ) if legend else dict(visible=False),
        xaxis=dict(gridcolor=PC["grid"], showgrid=True, linecolor="#E2E8F0",
                   tickfont_size=10, titlefont_size=11),
        yaxis=dict(gridcolor=PC["grid"], showgrid=True, linecolor="#E2E8F0",
                   tickfont_size=10, titlefont_size=11),
    )
    return fig
...
def plot_chromatogram(t, sig, tech_name, meta, run_number):
    ...
    fig.update_layout(
        title=dict(text=f"<b>Chromatogramme — {tech_name}</b> | Run #{run_number}", font_size=13),
...
def plot_mrm_detail(tech_name, run_number):
    ...
    fig.update_layout(
        title=dict(text=f"<b>Transitions MRM — {tech_name}</b> | Run #{run_number}", font_size=13),
    )
    for i in range(1, 5):
        try:
...
def plot_hrms_ppm(tech_name, run_number, drift_df):
    ...
    fig.update_layout(
        title=dict(text="<b>Évolution erreur de masse (ppm)</b> — Historique 30 runs", font_size=13),
...
def plot_drift_trends(drift_df, tech_name, run_number):
    ...
    fig.update_layout(
        title=dict(text="<b>Métriques de dérive instrumentale — 30 runs</b>", font_size=13),
    )
    return fig
...
def plot_mrm_ratios(drift_df, tech_name, run_number):
    ...
    apply_fig_style(fig, height=360)
    fig.update_layout(
        title=dict(text="<b>Évolution ratios Q/q par composé — Historique 30 runs</b>", font_size=13),
        xaxis_title="Numéro de run",
...
def plot_dashboard_overview(drift_df, tech_name, run_number):
    ...
    fig1.update_layout(
        title=dict(text="<b>RT Shift moyen (min)</b>", font_size=12),
    ...
    fig2.update_layout(
        title=dict(text=f"<b>Bruit de fond ({p['unit']})</b>", font_size=12),
    ...
    if p["type"] == "hrms":
        ...
        fig3.update_layout(
            title=dict(text="<b>Erreur masse (ppm) — Composé 1</b>", font_size=12),
        ...
        apply_fig_style(fig3, height=280, legend=False)
        fig3.update_layout(
            title=dict(text="<b>Largeur de pic FWHM (min)</b>", font_size=12),
...
def page_drift(tech_name, run_number):
    ...
    with st.expander("ℹ️ Algorithme de détection — Comprendre les règles", expanded=False):
        st.markdown("""
**ChromatoIA applique 3 règles de détection complémentaires :**

| Règle | Déclencheur | Niveau | Logique |
|---|---|---|---|
| **1 — Seuil absolu** | Métrique > seuil prédéfini | ⚠️ Warning | Réactive : détecte une dérive déjà présente |
| **2 — Tendance 5 runs** | Pente monotone sur 5 runs consécutifs (R² > 0.80) | ⚠️ Prédictif | Détecte une dérive **avant** le dépassement de seuil |
| **3 — Accélération** | Vitesse de dérive ×2 entre première et deuxième moitié | 🔴 Critique | Détecte une dégradation soudaine |

**Métrique principale par technique :**
- LC/GC classique : RT shift, FWHM, asymétrie
- LC-MS / GC-MS : intensité relative, bruit de fond
- MRM : ratio Q/q par composé
- HRMS : erreur de masse en ppm
        """)
    ...
    if p["type"] in ["classical", "ms"]:
        ...
        with col_b:
            ...
            fig_rt.update_layout(title=dict(text="<b>RT Shift moyen</b>", font_size=12),
                                 xaxis_title="Run", yaxis_title="ΔRT (min)")
        ...
        fig_rt2.update_layout(title=dict(text="<b>RT Shift moyen</b>", font_size=12),
                              xaxis_title="Run", yaxis_title="ΔRT (min)")
...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown(
            ...
            f'display:inline-block; margin-bottom:4px;">{fam} · {TECHNIQUES[tech_name]["type"].upper()}</div>',
            unsafe_allow_html=True
        )

        st.markdown("##### NUMÉRO DE RUN")
        run_number = st.slider(
            "Run courant",
        ...
        if run_number >= drift_start:
            ...
            st.markdown(
                ...
                f'border-radius:8px; font-size:11px; font-weight:500; margin-top:4px;">'
                f'✅ Instrument nominal (dérive dans ~{runs_to_drift} runs)</div>',
                unsafe_allow_html=True
            )
        ...
        page = st.radio(
            "Page",
            ["🏠 Dashboard", "📊 Analyse Chromatogramme",
             "📈 Détection de Dérive", "📋 Rapport QC", "ℹ️ À Propos"],
            label_visibility="collapsed",
        )
        ...
        <div style="font-size:10px; color:#94A3B8; line-height:1.6;">
            ...
            <strong>Colonne</strong><br>{col_inst}
        </div>""", unsafe_allow_html=True)
...
def main():
    ...
    if   "Dashboard"       in page: page_dashboard(tech_name, run_number)
