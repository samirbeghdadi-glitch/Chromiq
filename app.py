...
def page_parc_analytique():
    ...
    # On utilise run 25 comme "état actuel" par défaut
    DEFAULT_RUN = 25

    park_data = []
    for tech_name, p in TECHNIQUES.items():
        drift_df = calculate_drift_history(tech_name)
        anomalies = detect_anomalies(drift_df.head(DEFAULT_RUN), tech_name)
        n_crit = sum(1 for a in anomalies if a["level"] == "critical")
        n_warn = sum(1 for a in anomalies if a["level"] == "warning")

        d_run = max(0, DEFAULT_RUN - p["drift_start"])
        if n_crit > 0:
            status = "CRITIQUE"
        elif n_warn > 0:
            status = "WARNING"
        ...
        rt_shift = drift_df.loc[DEFAULT_RUN-1, "RT_shift_min"] if DEFAULT_RUN <= len(drift_df) else 0
        park_data.append({
            "tech": tech_name,
            "instrument": p["instrument"],
            "family": p["family"],
    ...
    for i in range(0, len(park_data), 2):
        ...
        for j, col in enumerate([col_left, col_right]):
            ...
            with col:
                ...
                anomaly_txt = d["alert_msg"][:60] + "..." if len(d["alert_msg"]) > 60 else d["alert_msg"]
                if d["status"] == "OK":
...
<div style="background:{cfg['bg']}; border:2px solid {cfg['border']};
  ...
  <div style="font-size:11px; color:#374151; line-height:1.5; margin-bottom:12px;">
    {anomaly_txt}
  </div>
  <div style="display:flex; gap:12px; font-size:10px; color:#64748B;">
    <span>📊 {drift_info}</span>
    <span>🔢 {d['n_crit']} crit · {d['n_warn']} warn</span>
  </div>
</div>"""
