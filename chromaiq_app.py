...
def load_run(run_id: str):
    """Fetch a run by ID — get_run is @st.cache_data, so this is fast after first call."""
    cfg = next(r for r in RUNS_CFG if r["id"] == run_id)
    return get_run(cfg["drift"], cfg["seed"])
...
run_cfg = RUNS_CFG[st.session_state.run_idx]
t_cur, y_cur, cmpds_cur, ns_cur, peaks_cur = load_run(run_cfg["id"])
t_ref, y_ref, _, _, peaks_ref = load_run("REF")
drift = run_cfg["drift"]
